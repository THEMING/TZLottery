...
<script>
	...
	alert("提示：请正确填写手机号码!")
	return false;}
	location.href="/customer/MyInfo.htm?action=sjyz&mobileNo="+$("#mobileNo1").val()
	}
</script>
</head>
<body>
        ...
        <div id="main1" class="main">
	        ...
	        <table cellpadding="0" cellspacing="0" border="0" width="669" height="175"> 
				...
				<tr> 
					...
					<s:if test="customer.bound==null || customer.bound!='bound'">
					<td>
						<input type="text" class="text" id="mobileNo1" name="mobileNo" value="${customer.mobileNo}"></input>
						<input type="button" value="手机号码绑定" onclick="bound()"/>
					</td>
					</s:if>
					<s:else>
					<td>
