...
<form id="quer" action="/oss/term/manageSendOrder.htm?action=listJCLQ" method="post">
...
<form id="form2" action="/oss/basketball/manageJCLQOpenPrize.aspx" method="post">
	...
	<table id="listTable">
		...
		<tr>
		    ...
		    <td ><div align="center">让分</div></td>
		    <td ><div align="center">大小分</div></td>
		    <td ><div align="center">客队</div></td>
		    <td ><div align="center">开赛时间</div></td>
		    <td ><div align="center">状态</div></td>
		    <td ><div align="center">全场比分</div></td>
		    <td ><div align="center">胜负(SP)</div></td>
		    <td ><div align="center">让分胜负(SP)</div></td>
		    <td ><div align="center">大小分(SP)</div></td>
		...
		<s:iterator value="page.result" id="match">
			<tr>
				...
				<td>${match.danguanRangFen}</td>
				<td>${match.danguanDaXiaoQiu}</td>
				<td>${match.awaryTeam}</td>
				<td><s:date name="#match.matchTime" format="yyyy-MM-dd HH:mm"/></td>			
				<td>${match.status}</td>
				<td>${match.wholeScore}</td>
				<c:choose>
				<c:when test="${match.sfResult == '1'}">
				<td>主胜(${match.spSf})</td>
				</c:when>
				<c:when test="${match.sfResult == '2'}">
				<td>主负(${match.spSf})</td>
				</c:when>
				<c:otherwise>
				<td>${match.spSf}</td>
				...
				<c:choose>
				<c:when test="${match.rfsfResult == '1'}">
				<td>主胜(${match.spRangfenSf})</td>
				</c:when>
				<c:when test="${match.rfsfResult == '2'}">
				<td>主负(${match.spRangfenSf})</td>
				</c:when>
				<c:otherwise>
				<td>${match.spRangfenSf}</td>
				</c:otherwise>
				</c:choose>
				<c:choose>
				<c:when test="${match.dxfResult == '1'}">
				<td>大分(${match.spDxf})</td>
				</c:when>
				<c:when test="${match.dxfResult == '2'}">
				<td>小分(${match.spDxf})</td>
				</c:when>
				<c:otherwise>
				<td>${match.spDxf}</td>
				</c:otherwise>
