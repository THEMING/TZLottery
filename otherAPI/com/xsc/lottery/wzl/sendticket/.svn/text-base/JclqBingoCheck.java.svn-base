...
@Service("JclqBingoCheckService")
@Transactional
public class JclqBingoCheck
{
    @Autowired
    private MatchArrangeService matchArrangeService;
    //计算Ticket的结果。
    //多关让分和多关大小分的玩法，让分和大小分是不固定的，需要根据Ticket 中的值来计算。
    private String CalculateTicketResult(String matchNo, String szSpecial, PlayType playtype, int nIndex)
    {
    	String szResult = "";
    	try
    	{
	    	...
	    	{
        		String szRangFen = szAllRangFen.split("\\|")[nIndex];
        		float fRangFen = Float.parseFloat(szRangFen);
	    		if(fHomeScore + fRangFen > fAwayScore)
	    		{//主胜
	    			szResult = "3";
	    		}
	    		else//主负
	    	...
	    	{
    			float fDiff = fHomeScore - fAwayScore;
    			if(fDiff > 0)
    			{//主胜
    				if(fDiff <= 5.5f)
    					szResult = "01";
	    	...
	    	else if(playtype.equals(PlayType.DXF))
	    	{
	    		String szRangFen = szAllRangFen.split("\\|")[nIndex];
	    		float fRangFen = Float.parseFloat(szRangFen);
	    		float fTotalScore = fAwayScore + fHomeScore;
	    		if(fTotalScore > fRangFen)
	    		{//大分
	    			szResult = "3";
	    		}
	    		else//小分
    ...
    public WinDescribeTicket execute(Ticket ticket) throws Exception
    {
        ...
        for (int i=0; i<matchBets.length; i++)
        {
        	...
        	String result = "";
        	//加入竞彩蓝球混合过关的开奖
        	if(ticket.getPlayType() == PlayType.JL_HH)
        	{
        		//SF>140601001
        		matchNo = matchNo.split("\\>")[1]; //场次编号
        		playType = PlayType.valueOf(matchNo.split("\\>")[0]);//投注方式
        		result = matchArrangeService.getResultByMatchNo(matchNo, playType, LotteryType.竞彩篮球);
        	}
        	else
        	{
        	...
        	{//单关的胜分差用程序计算结果
        		if(playType.equals(PlayType.SFC))
        		{
            		String exResult = CalculateTicketResult(matchNo, ticket.getTicketSpecial(), playType, i);
            		if(!exResult.equals(""))	specificRes = exResult;
        		}
...
private BigDecimal calculateMc1Tax(List<WinDesc> winList, int m, int index)
{
	...
	int length;
	BigDecimal money,tax,taxsum,currentratio;
	int special;
